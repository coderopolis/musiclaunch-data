// ---------------------------------------------------------------------------
// Influencer Resource Updater — uses Claude API with web search to
// verify/update influencer & playlist promotion resource pricing and
// details quarterly.
//
// Two-phase approach:
...
const RESOURCE_FILE = path.join(__dirname, '..', 'influencer-resources.json');
...
async function main() {
  console.log('Reading current influencer-resources.json...');
  ...
  const researchResponse = await callClaude({
    ...
    messages: [
      {
        role: 'user',
        content: `You are verifying influencer marketing, playlist promotion, and music discovery platform resources for an indie music app. Search the web to check each of these ${resourceCount} resources for current pricing, availability, and whether they're still active.
...
Also note if you discover 1-3 NEW noteworthy platforms for:
- Music influencer marketplaces / submission platforms
- Playlist pitching services
- TikTok / Instagram Reels music promotion tools
- Smart link or pre-save tools for independent musicians
...
Be concise — just the facts for each resource.`
  ...
  const jsonResponse = await callClaude({
    ...
    messages: [
      {
        role: 'user',
        content: `Based on the research findings below, produce an updated version of the influencer resources JSON.
